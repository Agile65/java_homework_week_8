...
*/
public class Program_5Palindrome {
    //static method
    public static boolean isPalindrome(int number){
        ...
        temp=number;//storing original number in temp variable
        while(number>0){
            //stores the last digit
            lastDigit=number%10;
            reverse=reverse*10+lastDigit; //stores reverse of the number in reverse variable
            number=number/10;
        }
        return reverse==temp;
    }
    ...
    public static void main(String[] args) {
        ...
        boolean ans=isPalindrome(scanner.nextInt());
        if(ans){
